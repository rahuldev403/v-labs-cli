...
// 1. Import the delay function alongside your batching utility
...
export async function getDecisionMakers(domains) {
  ...
  const results = await processInBatches(domains, 1, async (domain) => {
    try {
      // 2. Add a 2.5-second cooldown before firing the next request
      console.log(
        `  ⏳ Cooling down for 2.5s to respect Prospeo rate limits...`,
      );
      await delay(2500);
  ...
  console.log(
    `\n✅ Extracted ${validExecutives.length} high-level executives.`,
  );
